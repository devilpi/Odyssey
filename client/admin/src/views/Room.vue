    ...
    export default {
        ...
        methods: {
            ...
            create () {
                ...
                , function (success) {
                    if (!success) {
                        ...
                        room.$socket.emit('join',room.user_id,room.username,room.room
                            , function (success) {
                                if (success) {
                                    console.log(success)
                                } else {
                        ...
                        room.userinfo.user_id = room.user_id
                        room.userinfo.username = room.username
                        room.userinfo.roomname = room.room
                        axios.post('http://47.100.165.222',room.userinfo)

                        room.gameLink = 'http://47.100.165.222/index.html?userid='+room.user_id
                         window.location.href = room.gameLink
                         window.location.replace(room.gameLink)
                    }
                    room.loading = false
                })
            },
            join () {
                ...
                let room = this
                this.$socket.emit('join',room.user_id,room.username,this.room,
                    function (success) {
                    if (success) {
                        console.log(success)
